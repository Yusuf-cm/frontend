...
import { FiSend, FiCheckCircle, FiAlertCircle, FiMapPin, FiPhone, FiMail, FiClock } from &apo:react-icons/fi&apo:;
import { motion } from &apo:framer-motion&apo:;
import dynamic from &apo:next/dynamic&apo:; // <-- FIX #1: IMPORT DYNAMIC

// Define the Map component outside the main component to avoid re-creating it on every render.
const Map = dynamic(() => import(&apo:@/components/Map&apo:), { 
...
const ContactPage = () => {
    ...
    const [status, setStatus] = useState(&apo:&apo:); // &apo:success&apo:, &apo:error&apo:, &apo:loading&apo:
    ...
    const handleChange = (e) => {
        ...
        if (errors[name]) {
            setErrors(prev => ({ ...prev, [name]: &apo:&apo: }));
        }
    };

    const validateForm = () => {
        const newErrors = {};
        if (!formData.name.trim()) newErrors.name = &apo:Name is required&apo:;
        if (!formData.email.trim()) newErrors.email = &apo:Email is required&apo:;
        else if (!/^\S+@\S+\.\S+$/.test(formData.email)) newErrors.email = &apo:Invalid email format&apo:;
        if (!formData.message.trim()) newErrors.message = &apo:Message is required&apo:;
        else if (formData.message.trim().length < 10) newErrors.message = &apo:Message should be at least 10 characters&apo:;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setIsSubmitted(true);
                setFormData({ name: &apo:&apo:, email: &apo:&apo:, message: &apo:&apo: });
            } else {
                setStatus(&apo:error&apo:);
            }
        } catch (error) {
            console.error(&apo:Submission error:&apo:, error);
            setStatus(&apo:error&apo:);
        }
    };
        ...
        <div className="bg-gradient-to-b from-gray-50 to-white">
            <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
                <div className="text-center">
                    ...
                    >
                        We&apo:d love to hear about your project or answer any questions
                    </motion.p>
                </div>
                ...
                <div className="mt-16 grid grid-cols-1 lg:grid-cols-2 gap-16">
                    ...
                    <div className="space-y-8">
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="pt-8">
                            <div className="bg-gray-100 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-gray-900 mb-3">We&apo:re Here to Help</h3>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                        ...
                        {isSubmitted && status === &apo:success&apo: ? (
                            <div className="mt-8 bg-green-50 rounded-xl p-6 text-center">
                                ...
                                <p className="mt-2 text-green-700">Thank you for contacting us. We&apo:ll get back to you as soon as possible.</p>
                            ...
                            <form onSubmit={handleSubmit} className="mt-6 space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value={formData.name} onChange={handleChange} placeholder="John Doe" className={`mt-1 block w-full px-4 py-3 border ${errors.name ? &apo:border-red-500&apo: : &apo:border-gray-300&apo:} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent`} />
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" value={formData.email} onChange={handleChange} placeholder="john@example.com" className={`mt-1 block w-full px-4 py-3 border ${errors.email ? &apo:border-red-500&apo: : &apo:border-gray-300&apo:} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent`} />
                                ...
                                <div>
                                    ...
                                    <textarea name="message" id="message" rows="5" value={formData.message} onChange={handleChange} placeholder="How can we help you?" className={`mt-1 block w-full px-4 py-3 border ${errors.message ? &apo:border-red-500&apo: : &apo:border-gray-300&apo:} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent`}></textarea>
                                    {errors.message && <p className="mt-1 text-red-600 text-sm flex items-center"><FiAlertCircle className="mr-1" /> {errors.message}</p>}
                                </div>
                                <div>
                                    <button type="submit" disabled={status === &apo:loading&apo:} className="w-full flex justify-center items-center py-3 px-6 border border-transparent rounded-lg shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400">
                                        {status === &apo:loading&apo: ? &apo:Sending...&apo: : <><FiSend className="mr-2" />Send Message</>}
                                    </button>
                                </div>
                                {status === &apo:error&apo: && <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded"><p className="text-sm text-red-700">Something went wrong. Please try again later.</p></div>}
                            </form>
                        )}
